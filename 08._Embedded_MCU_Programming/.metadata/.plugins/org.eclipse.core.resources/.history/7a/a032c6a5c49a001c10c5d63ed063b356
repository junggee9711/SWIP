...
{
    /* Reset PC1 in IOCR0*/
    PORT10_IOCR0 &= ~((0x1F) << PC0);

    /* Set PC1 with push-pull(2b10000) */
    PORT10_IOCR0 |= ((0x10) << PC0);
}
...
void init_ERU(void)
{
    ...
    SCU_EICR1 &= ~(((0x7) << EXIS0) | ((0x7) << EXIS1));             // External input 1 is selected
    ...
    SCU_EICR1 &= ~(((0x7) << INP0) |((0x7) << INP1));              // An event from input ETL 2 triggers output OGU 0
    SCU_EICR1 |= ((0x1) << INP1);

    SCU_IGCR0 &= ~(((0x3) << IGP0)|((0x3) << IGP1));              // IOUT(0) is activated in response to a trigger event
