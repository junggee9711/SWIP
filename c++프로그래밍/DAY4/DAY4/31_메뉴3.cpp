...
// PopupMenu : 선택하면 다른 메뉴 (하위메뉴) 가 열리는 메뉴
...
public:
	...
	void addMenu(MenuItem* p) { v.push_back(p); }		// v.push_back(p) => 벡터 v에 p를 붙임
	...
	void command()
	{
		while (1)
		{
			...
			if (cmd == sz + 1) // 종료 메뉴 선택시
				break;		   // 또는 return (command() 함수를 나감)
...
int main()
{
	...
	PopupMenu pm("카메라메뉴");
	pm.addMenu(&m1);
	pm.addMenu(&m2);	// 팝업메뉴에 m1, m2 를 붙임.
